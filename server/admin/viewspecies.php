...
if(array_key_exists("submit",$_POST) && $_POST["submit"]=="Add"){
	...
	# Execute query
	$result = pg_prepare($dbconn, "insertspecies", "INSERT INTO species (name,short) VALUES ($1,$2)")
		or die('Failed to prepare insert query: ' . pg_last_error());
	$data = array($_POST["name"],$_POST["short"]);
	$result = pg_execute($dbconn, "insertspecies", $data)
...
# Delete Corp
	...
	# Execute query
	$result = pg_prepare($dbconn, "deletespecies", "DELETE FROM species WHERE id = $1")
		or die('Failed to prepare insert query: ' . pg_last_error());
	$result = pg_execute($dbconn, "deletespecies", array($_POST["id"]))
		or die('Failed to execute insert query: ' . pg_last_error());
...
# Import data from CSV file
	...
	$result = pg_prepare($dbconn, "insertnewspecies", "INSERT INTO species (name,short) VALUES ($1,$2)")
		or die('Failed to prepare insert query: ' . pg_last_error());
	...
	while (($data = fgetcsv($f, 0, ",")) !== FALSE) {
		...
		$result = pg_execute($dbconn, "insertnewspecies", array($data[0],$data[1]))
...
# Export data to CSV file
	...
	# Setup query
	$result = pg_query($dbconn, "SELECT name,short FROM species")
		or die('Failed to perform species select query: ' . pg_last_error());
	# Pull data from DB
	$data = array();
...
<table>
        ...
        <tr><td>Short Name</td><td><input type="text" name="short" /></td></tr>
	<tr><td><input type="submit" name="submit" value="Add" /></td><td /></tr>
</table></form><br /><hr /><br /><?php
# Display list of speciess
if(array_key_exists("page",$_GET))
	$page = (int)$_GET["page"];
else
	$page = 0;
$offset = (int)($page * 100);
if($page>0){?><a href="viewspeciess.php?page=<?php echo $page-1; ?>">Prev Page</a> &nbsp; <?php } ?><a href="viewspeciess.php?page=<?php echo $page+1; ?>">Next Page</a><br /><?php
echo "<table><tr><th>Name</th><th>Short Name</th><th />";
$result = pg_query($dbconn, "SELECT id,name,short FROM species 
...
while ($row = pg_fetch_row($result)) {
	...
	echo "</td>";
	//echo '<td><a href="editspecies.php?id='.$row[0].'">EDIT</a></td>'; 
	echo "</tr>";
}
echo "</table>";
?></html>
